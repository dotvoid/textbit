...
describe('withBlockBoundaryGuard - delete strips text, keeps structure', () => {
  ...
  test('through-the-block selection (both outside) deletes the block too', () => {
    // Anchor in "outside", focus past the blocks (we add a tail node).
    ...
    const ids = editor.children.map((c) => (c as { id: string }).id)
...
describe('withBlockBoundaryGuard - chained invocation paths', () => {
  test('deleteFragment with direction backward (slate-react Backspace path)', () => {
    // slate-react routes an expanded-selection Backspace to
    // editor.deleteFragment({ direction: 'backward' }), not to deleteBackward
    // (Slate's deleteBackward no-ops on expanded selections). The guard must
    // handle the backward direction the same as the default.
    const editor = makeEditor(makeChildren())
    editor.selection = {
      anchor: { path: [0, 0], offset: 3 },
      focus: { path: [1, 0, 0], offset: 4 }
  ...
  test('typing places the caret right after the inserted character', () => {
    const editor = makeEditor(makeChildren())
    editor.selection = {
      anchor: { path: [0, 0], offset: 3 },
      focus: { path: [1, 0, 0], offset: 4 }
    ...
    // 'x' lands at the start of "ion", caret should be after it.
    expect(editor.selection).toEqual({
...
describe('withBlockBoundaryGuard - nested blocks', () => {
  test('crossing the inner block boundary strips text and keeps both blocks', () => {
    // Outer block wraps a 'text' sibling and an inner block; inner block has
    // its own 'text' child. Selection runs from outer's text into inner's text.
    const editor = makeEditor([
      {
        id: 'outer',
        type: 'test/outer',
    ...
    editor.selection = {
      anchor: { path: [0, 0, 0], offset: 2 }, // "ou|ter"
      focus: { path: [0, 1, 0, 0], offset: 3 } // "inn|er"
    }

    editor.deleteFragment()

    // Both blocks stay; only the spanned text shrinks.
    expect(text(editor, [0, 0, 0])).toBe('ou')
    expect(text(editor, [0, 1, 0, 0])).toBe('er')
  })
})
