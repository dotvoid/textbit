 ...
 *    (no merge), then collapse to the selection focus.
 *  - `insertText` / plain-text paste via `insertData` - delete the selected
 *    text the same way, then insert the new text at the selection end.
 *  - rich paste (slate fragment) / `insertFragment` - blocked.
 *  - `insertBreak` / `insertSoftBreak` - blocked.
 *
 * Selections fully inside one block, or fully outside any block, fall through
  ...
  editor.deleteFragment = (options) => {
    if (selectionCrossesBlockBoundary(editor)) {
      ...
      if (selection) {
        ...
        // when it ended outside. Matches the insert path's behaviour.
